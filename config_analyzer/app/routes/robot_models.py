...
def add_robot_model():
    if request.method == 'POST':
        ...
        if not name or not manufacturer:
            flash("Le nom et le fabricant sont obligatoires.", "error")
            return redirect(url_for('robot_models.add_robot_model'))

        new_robot_model = RobotModel(name=name, manufacturer=manufacturer)
...
def edit_robot_model(robot_model_id):
    ...
    if request.method == 'POST':
        ...
        robot_model.manufacturer = request.form['manufacturer']
